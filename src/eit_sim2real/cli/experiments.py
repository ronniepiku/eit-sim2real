...
@click.option(
    "--coarse-dataset",
    type=click.Path(exists=True, dir_okay=False),
    default="data/eit_dataset_mesh_c.mat",
    show_default=True,
    help="Coarse-mesh noisy dataset (refinement c).",
)
@click.option(
    "--baseline-dataset",
    type=click.Path(exists=True, dir_okay=False),
    ...
    show_default=True,
    help="Baseline noisy dataset (refinement d).",
)
@click.option(
    "--fine-mesh-dataset",
    type=click.Path(exists=True, dir_okay=False),
    ...
    show_default=True,
    help="Fine-mesh noisy dataset (refinement f).",
)
@click.option(
    "--output-dir",
    type=click.Path(file_okay=False),
...
def mesh_refinement(
    ...
    """Train and cross-evaluate CNNs across c/d/f mesh refinements.

    The command expects three matching noisy datasets generated with the
    same sampling protocol but different mesh refinements.
    """
    from eit_sim2real.experiments.mesh_refinement import main as mesh_main

    mesh_main(
        coarse_dataset=coarse_dataset,
