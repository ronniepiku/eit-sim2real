function [NF,SE] = calc_noise_figure( inv_model, hp, iterations, extraparam)
...
% $Id: calc_noise_figure.m 6942 2024-06-18 14:53:21Z aadler $
...
elseif nargin == 4
% NF = nf_calc_linear( inv_model, h_data, c_data, hp);
   h_data = hp;
   c_data = iterations;
   inv_model.hyperparameter.value= extraparam;
elseif nargin>=2 && numel(hp) == 1
...
if 0 % old code with random noise
   ...
   noise = 0.01*std(vhm)*speye(size(vhm,1));
   vhn= vhm*ones(1,size(vhm,1)) + noise;
end
...
function test0
    ...
    img = mk_image(imdl,1); vh=fwd_solve(img);
    img.elem_data(1:4)=1.1; vi=fwd_solve(img);
    NF=calc_noise_figure(imdl,vh,vi);
