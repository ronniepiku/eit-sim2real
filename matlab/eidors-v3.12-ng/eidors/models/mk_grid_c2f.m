...
% $Id: mk_grid_c2f.m 7040 2024-11-30 00:02:08Z bgrychtol $
...
%                         plot3(nv(:,1),nv(:,2),nv(:,3),'xb');
    ...
    vol = get_elem_volume(fmdl, 'no_c2f');
...
% Make a 3D grid from 2D model
% where the max and min enclose fmdl
function rmdl = rmdl_3D(rmdl, fmdl)
   fzmax = max(fmdl.nodes(:,3));
   fzmin = min(fmdl.nodes(:,3));
   rm_nodes = rmdl.nodes;
   oo = 0*rm_nodes(:,1) + 1;
   N = num_nodes(rmdl);
   % nodes at top + bottom
   rmdl.nodes = [ ...
      ...
      rmdl.nodes,oo*fzmax];
   rme = [
      rmdl.elems(:,[1,2,3,1]) + [N,0,0,0];
      rmdl.elems(:,[1,2,3,2]) + [N,N,0,0];
      rmdl.elems(:,[1,2,3,3]) + [N,N,N,0]];
   % put elem block together
   rme = reshape(rme,[],3,4);
   rme = permute(rme,[2,1,3]);
   rmdl.elems = reshape(rme,[],4);
   rmdl.coarse2fine = kron( ...
        rmdl.coarse2fine,[1;1;1]);
   return
% Test code to see block
   x = ones(6,1);
   rmdl.boundary = find_boundary(rmdl);
   x(3) = 2;
   img = mk_image(rmdl,x);
...
function do_unit_test
    ...
    clf
    do_case_tests;
    return % the tests below are more about performance than correctness
    do_realistic_test;
    ...
    for i = 1:30
        ...
        switch i
            ...
            case 4 % tet_edge v vox_edge
                ...
                unit_test_cmp(txt,size(intpts,1),0,0); % nothing
                [intpts, face2edge, face2intpt, edge2intpt] = test_tet_vedge_intersections(vox,tet);
                unit_test_cmp(txt,size(unique(intpts,'rows'),1),1,0); % 1 point
                [intpts, face2edge, face2intpt, edge2intpt] = test_tedge2vedge_intersections(vox,tet);
            ...
            case 5 % tet_edge on vox_face
                ...
                unit_test_cmp(txt,size(intpts,1),0,0); % nothing
                [intpts, face2edge, face2intpt, edge2intpt] = test_tet_vedge_intersections(vox,tet);
                unit_test_cmp(txt,size(unique(intpts,'rows'),1),3,0); % 3 points
                [intpts, face2edge, face2intpt, edge2intpt] = test_tedge2vedge_intersections(vox,tet);
            ...
            case 8
                ...
                unit_test_cmp(txt,size(intpts,1),0,0); % nothing
                [intpts, face2edge, face2intpt, edge2intpt] = test_tet_vedge_intersections(vox,tet);
                unit_test_cmp(txt,size(uniquetol(intpts,eps,'ByRows',true),1),1,0); % 1 point
                [intpts, face2edge, face2intpt, edge2intpt] = test_tedge2vedge_intersections(vox,tet);
                unit_test_cmp(txt,size(intpts,1),0,0); % nothing
                [fnode2vox] = test_tnode_in_vox(vox,tet);
                unit_test_cmp(txt,nnz(fnode2vox),1,0); % 1 point
            ...
            case 15 % common edge
                ...
                unit_test_cmp(txt,size(intpts,1),0,0); % nothing
                [intpts, face2edge, face2intpt, edge2intpt] = test_tet_vedge_intersections(vox,tet);
                unit_test_cmp(txt,size(unique(intpts,'rows'),1),1,1); % 1
                [intpts, face2edge, face2intpt, edge2intpt] = test_tedge2vedge_intersections(vox,tet);
                unit_test_cmp(txt,size(intpts,1),0,0); % nothing
                [fnode2vox] = test_tnode_in_vox(vox,tet);
                unit_test_cmp(txt,nnz(fnode2vox),1,0); % 1 point
            ...
            case 17 % edge on face
                ...
                unit_test_cmp(txt,size(intpts,1),0,0); % nothing
                [intpts, face2edge, face2intpt, edge2intpt] = test_tet_vedge_intersections(vox,tet);
                unit_test_cmp(txt,size(unique(intpts,'rows'),1),2,0); % 2
                [intpts, face2edge, face2intpt, edge2intpt] = test_tedge2vedge_intersections(vox,tet);
                unit_test_cmp(txt,size(intpts,1),0,0); % nothing
                [fnode2vox] = test_tnode_in_vox(vox,tet);
            ...
            case 18 % edge2edge only
                ...
                unit_test_cmp(txt,size(intpts,1),0,0); % nothing
                [intpts, face2edge, face2intpt, edge2intpt] = test_tet_vedge_intersections(vox,tet);
                unit_test_cmp(txt,size(unique(intpts,'rows'),1),2,0); % 2 points
                [intpts, face2edge, face2intpt, edge2intpt] = test_tedge2vedge_intersections(vox,tet);
                unit_test_cmp(txt,size(intpts,1),2,0); % 2 points
                [fnode2vox] = test_tnode_in_vox(vox,tet);
                unit_test_cmp(txt,nnz(fnode2vox),0,0); % nothing
            ...
            case 20 %face on face
                ...
                unit_test_cmp(txt,size(intpts,1),0,0); % nothing
                [intpts, face2edge, face2intpt, edge2intpt] = test_tet_vedge_intersections(vox,tet);
                unit_test_cmp(txt,size(uniquetol(intpts,2*eps,'ByRows',true),1),3,0); % 3 points
                [intpts, face2edge, face2intpt, edge2intpt] = test_tedge2vedge_intersections(vox,tet);
