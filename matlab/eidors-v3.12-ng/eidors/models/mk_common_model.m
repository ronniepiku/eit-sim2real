...
% $Id: mk_common_model.m 6918 2024-05-30 11:33:00Z bgrychtol $
...
elseif strcmp( str(2:3), '3c') || strcmp(str(2:3),'3t')
   ...
   elseif str(1)=='g'; xy_layers= 28; z_layers= linspace(-1.9,1.9,101);
   elseif str(1)=='h'; xy_layers= 32; z_layers= linspace(-2.2,2.2,121);
   else;  error(['don`t know what to do with option(1)=',str]);
   end
...
function inv2d= add_params_2d_mdl( params, n_elec, options);
    ...
    params.normalize_measurements = mdl_normalize('default');
...
function inv_mdl = mk_n3z_model( n_elec, options );
   if isempty(n_elec) || ~all(n_elec == [16,1])
      warning(sprintf(['You have requested an "n3z" model with [%d,%d] electrodes. ' ...
         'Note that these models always have 16 electrodes (16x1)'], n_elec));
   end
   inv_mdl= mk_n3r2_model( [16, 2], options);
   fwd_mdl= inv_mdl.fwd_model;
   renumber= [1:2:15; 18:2:32];
   fwd_mdl.electrode= fwd_mdl.electrode(renumber(:));
  
   [st, els]= mk_stim_patterns(16, 1, '{ad}','{ad}', options, 10);
   fwd_mdl.stimulation= st;
   fwd_mdl.meas_select= els;
...
function inv_mdl = mk_n3r2_model( n_elec, options );
   ...
   fmdl.normalize_measurements = mdl_normalize('default');
