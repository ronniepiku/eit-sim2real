...
% $Id: calc_grid_points.m 6903 2024-05-28 18:38:34Z bgrychtol $
...
if nargout == 2
   c2f = logical(sparse(size(pts,1), size(use_elem,1)));
   c2f(:, use_elem) = point2tet;
   c2f = c2f'; % the usual orientation
end
...
% val is sparse sometimes in octave ... not sure why?
val = reshape(full(val), [size(x) size(data,2)]);
return
