...
% $Id: show_fem_enhanced.m 6974 2024-10-29 19:43:09Z aadler $
...
switch nargin
    ...
    case 1
        ...
        if ischar(mdl) && strcmp(mdl,'refresh')
            refresh_current_axis; 
            return; 
        end
        options = [];
        % overwrite options from field, if it exists
        try options = ...
            mdl.show_fem;
        end
        try options = ...
            mdl.show_fem_enhanced;
        end
        try options = ...
            mdl.fwd_model.show_fem;
        end
        try options = ...
            mdl.fwd_model.show_fem_enhanced;
        end
    case 2
...
function [img, mdl, opts] = proc_params(mdl, src_opts)
    ...
    if (nargin == 2)
        if (isstruct(src_opts))
            ...
            opts = copy_field(opts, src_opts, 'body', 'triangle_alpha');
            
            if (isfield(src_opts, 'body'))
            end
...
function hh = draw_fem(img, mdl, opts)
    ...
    if (size(mdl.elems, 2) == 4)
        ...
        alpha_level = opts.body.triangle_alpha;
        triangle_alpha = alpha_level*ones(size(elems, 1), 1);
    ...
    if ~isempty(img)
        if (size(mdl.elems, 2) == 4) 
            ...
            if size(img_data,1) == size(mdl.elems,1)
                triangle_color = calc_colours(mdl.element2sub_element*...
                                    img_data, img);
            elseif size(img_data,1) == size(mdl.nodes,1)
                triangle_color = calc_colours(img_data, img);
            else
            ...
            alpha_level = opts.body.triangle_alpha;
            if size(img_data,1) == size(mdl.elems,1)
                alpha_idx =  triangle_alpha(mdl.boundary_to_sub_element_idx, :) < alpha_level;
                triangle_alpha(mdl.boundary_to_sub_element_idx(alpha_idx), :) = alpha_level;
            else
                alpha_idx = triangle_alpha(mdl.boundary_node_idx, :) < alpha_level;
                triangle_alpha(mdl.boundary_node_idx(alpha_idx), :) = alpha_level;
            end
        else
...
function hh = draw_triangles(faces, vertices, color_data, alpha_data, ...
    ...
    elseif (size(alpha_data, 1) == size(vertices, 1))
        face_alpha = 'interp';  
        alpha_data_mapping = 'scaled';  % why is this needed??
    else
    ...
    hh = patch('Faces', faces, 'Vertices', vertices, ...
               'AlphaDataMapping', alpha_data_mapping, ...
               'CDataMapping', 'direct', ...
...
function mdl = find_sub_elements(mdl)
    if (~isfield(mdl, 'sub_elements'))
        ...
        % in case we get vertex data we need the boundary nodes
        mdl.boundary_node_idx  = (unique(mdl.boundary(:)));
...
function do_unit_test
   ...
   subplot(4,4,1); show_fem_enhanced(img.fwd_model,[0,0,1]) 
   title('regular mesh numbered');

if ~ver.isoctave 
   imgn = rmfield(img,'elem_data');
   imgn.node_data= 3*sin(linspace(-5,5,num_nodes(img))');
   subplot(4,4,9); show_fem_enhanced(imgn) 
   title('interpolated node colours');
end

   img2(1) = img; img2(2) = img;
   subplot(4,4,2); show_fem_enhanced(img,[1]);
   title('colours with legend');
   subplot(4,4,3); show_fem_enhanced(img2,[0,1]);
   title('colours with legend');
   img.calc_colours.mapped_colour = 0; % USE RGB colours
   subplot(4,4,4); show_fem_enhanced(img,[0,1,1]);
   title('RGB colours');
   subplot(4,4,4); show_fem_enhanced(img);
   title('RGB colours');

   img.elem_data = [1:10];
   subplot(4,4,12);show_fem_enhanced(img); %Should show grey
   title('error -> show grey');

if ~ver.isoctave
   imgn.calc_colours.mapped_colour = 0; % USE RGB colours
   subplot(4,4,10);show_fem_enhanced(imgn,[0,1]) 
   title('interpolated node colours');


   subplot(4,4,11);hh=show_fem_enhanced(imgn); set(hh(1),'EdgeColor',[0,0,1]);
   ...
   subplot(4,4,5); show_fem_enhanced(img3.fwd_model) 
   subplot(4,4,6); show_fem_enhanced(img3,[1])
   subplot(4,4,7); show_fem_enhanced(img3,[1,1])
   subplot(4,4,8); show_fem_enhanced(img3,[1,1,1])

   img3.elem_data(:) = 1;
   opts.viewpoint = struct('az',10,'el',10);
   subplot(4,4,13); show_fem_enhanced(img3,opts);
   opts.edge.width = 0;
   subplot(4,4,14); show_fem_enhanced(img3,opts);
   
   img3.fwd_model.show_fem_enhanced = opts;
   subplot(4,4,15); show_fem_enhanced(img3);
