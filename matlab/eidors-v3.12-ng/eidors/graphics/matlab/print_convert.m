...
% $Id: print_convert.m 6689 2024-03-19 16:04:23Z bgrychtol $
%
...
elseif ischar(pp.pagesize)
    ...
    pp = set_prop(pp, ...
        'PaperPositionMode','manual', ...
        'PaperSize', pp.pagesize+1, ... % big enough
        'PaperPosition', pos);
end
...
% Old options
   ...
   opt = varargin{1};
   if ischar(opt)
      for i = 1:length(varargin); opt = varargin{i}; parseok=false;
          ...
          val =regexp(opt,'-p(\d+)x(\d+)','tokens');
          if ~isempty(val);
             pp.pagesize = [str2num(val{1}{1}), str2num(val{1}{2})];
             parseok=true;
          end

          val =regexp(opt,'-ssf(\d+)','tokens');
          if ~isempty(val);
             pp.supersampling_factor = str2num(val{1}{1});
             parseok=true;
          end

          if ~parseok
             eidors_msg('print_convert: option (%s) not understood',opt,0)
          end
      end
   elseif isstruct(opt)
...
function do_unit_test
   fprintf('Doing UNIT_TEST for print_convert\n');
   ...
   print_convert pc05.png '-p10x20'
   print_convert pc05.png '-pauto'
   print_convert pc06.png '-r100' '-p10x20'

% No longer supported ... not useful
