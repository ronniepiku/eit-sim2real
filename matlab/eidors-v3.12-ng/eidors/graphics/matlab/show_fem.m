...
% $Id: show_fem.m 7029 2024-11-28 20:35:24Z bgrychtol $
...
function [img,mdl,opts] = proc_params( mdl, options );
   ...
   mdl = remove_instrument_electrodes(mdl);
...
for e=1:length(mdl.electrode)
    ...
    elseif isfield(mdl.electrode(e),'nodes') && ~isempty(elece.nodes)
        ...
        vx = vx(idx);
        vy = vy(idx);
    elseif isfield(elece,'pos') && ~isempty(elece.pos)
...
if num_lights == 0 % matlab has a maximum of 8
   camlight('left');
end
