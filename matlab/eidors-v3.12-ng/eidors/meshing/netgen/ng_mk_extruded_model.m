...
% $Id: ng_mk_extruded_model.m 7012 2024-11-26 15:52:57Z bgrychtol $
...
copt.fstr = 'ng_mk_extruded_model';
...
function [elecs, centres] = parse_elecs(elec_pos, elec_shape, tank_shape, hig, is2D )
   ...
   if size(elec_pos,1) == 1
       ...
       if is2D
          % create an electrode at half height
          elec_pos(:,3) = hig/2;
       end
...
function do_unit_test_basic;
    ...
    ng_write_opt('MSZBRICK',[-400,400,-400,400,120,180,40]);  
    fmdl = ng_mk_extruded_model({300,a,[4,25]},[16,1.11,150],[1]);
    show_fem(fmdl);

function do_unit_test_human_thorax
% load CT2
