...
% $Id: gmsh_mk_fwd_model.m 7004 2024-11-24 15:18:55Z aadler $
...
fwd_mdl = mdl_normalize(fwd_mdl, 'default'); % also orders fields


% build fwd_model structure
function mdl= construct_fwd_model(srf,vtx,simp,bc, name, ...
    stim_pattern, eprefix, z_contact, fc, phys_names)
...
for ver = vers(:)'
    ...
    fmdl2d.elems = uint32([2,5,1;1,5,4;3,5,2;,4,5,3]);
    fmdl2d.boundary = uint32([1,2;1,4;2,3;3,4]);
    fmdl2d.boundary_numbers = uint32(ones(4,1)*5);
    fmdl2d.gnd_node = 1;
    ...
    fmdl2d.electrode(1).nodes = uint32([1,4])';
    fmdl2d.electrode(2).nodes = uint32([2,3])';
    fmdl2d.type = 'fwd_model';
    fmdl2d.mat_idx = {[1:4]'};
    fmdl2d.mat_names = {'main'};

    fmdl3d = fmdl2d;
    fmdl3d.electrode(1).nodes = uint32([1,2,3,4,9])';
    fmdl3d.electrode(2).nodes = uint32([5,6,7,8,10])';
    fmdl3d.mat_idx = {[1:24]'};
    fmdl3d.boundary_numbers = uint32([ones(4,1)*13; ones(4,1)*14]);
    fmdl3d.boundary = uint32([
         2     1     9
         1     3     9
         4     2     9
         3     4     9
   ...
   fmdl2d = mdl_normalize(fmdl2d,'default');
   fmdl3d = mdl_normalize(fmdl3d,'default');

    % Test 2D and 3D parsing
    [fmdl,mat_ind] = gmsh_mk_fwd_model( ...
        fullfile(selfdir, ['tests/box-' ver '.msh']), fmdl2d.name, 'elec#', stim, elec.z_contact );
    if DEBUG
        for x = fields(fmdl)'
            x = x{1};
            if ~unit_test_cmp(['2d v' ver ' fmdl.' x],fmdl.(x),fmdl2d.(x))
               keyboard
            end
        end
        fmdl
    ...
    [fmdl,mat_ind] = gmsh_mk_fwd_model( ...
        fullfile(selfdir, ['tests/cube-' ver '.msh']), fmdl3d.name, 'elec#', stim, elec.z_contact );
    if DEBUG
        for x = fields(fmdl)'
            x = x{1};
            if ~unit_test_cmp(['3d v' ver ' fmdl.' x],fmdl.(x),fmdl3d.(x))
            end
        end
                keyboard
        fmdl
