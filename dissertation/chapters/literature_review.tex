...
The key representation here is the \emph{voltage difference vector}: the element-wise difference between contact and no-contact baseline measurements. This 208-dimensional vector encodes spatial touch information without explicit reconstruction and is the input to all classifiers.
...
Prosthetic systems operate within tight power, weight and computational budgets. \textcite{wang2020} reduce energy use through adaptive measurement, and \textcite{zhang2022fista} argue that classical algorithms (FISTA, $<$30 iterations) suit embedded deployment better than deep learning---constraints that motivate comparing lightweight classifiers against deeper CNNs here. The challenge extends beyond computation: \textcite{lee2020} note that greater sensor density means more wiring and reduced flexibility, and \textcite{jiang2023} that interconnects limit packing density. \gls{eit} addresses this directly by sensing distributively from few boundary electrodes \parencite{silvera-tawil2015}, at the cost of an ill-posed reconstruction.

\subsection{Long-term Reliability and Drift}
\label{sec:lit-deployment-drift}

Long-term stability is a prerequisite for deployment, yet most studies evaluate only short-term performance. \textcite{kim2019} observed gradual degradation over 10,000 cycles from fatigue and conductivity change, confirming drift as unavoidable rather than merely a risk; \textcite{costacornella2023} show self-healing materials restore function after damage, though healing may itself alter calibration. No study proposes learning-based compensation for progressive drift---a significant gap, since frequent recalibration is impractical in daily prosthetic use.
