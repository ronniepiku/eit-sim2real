...
def generate_ablation_report(
    ...
    if not single.empty:
        ...
        for _, row in single_sorted.iterrows():
            ...
            lines.append(
                f"| {label} | {row['test_acc']:.4f} | "
                f"{row['test_f1']:.4f} | {impact} |"
            )
    ...
    if not subset_df.empty:
        ...
        if not full_subset.empty:
            best_order = full_subset.loc[full_subset["test_acc"].idxmax()]
            lines.append(
                f"\n3. **Optimal noise application order**: "
                f"{best_order['noise_order']}"
            )
            lines.append(
    ...
    if run_severity_sweep and model_name == "cnn1d":
        ...
        for comp in NOISE_COMPONENTS:
            ...
            for sev in SEVERITY_LEVELS:
                ...
                y_pred = _predict_cnn(sweep_model, X_te, device=device)
