...
def generate_report(df: pd.DataFrame, output_dir: Path, runtime_s: float) -> None:
    ...
    report_lines.append(
        "| Dataset | Fixed Noisy→Noisy | Augmented→Noisy | Mixed→Noisy | "
        "Mixed→Clean |"
    )
    report_lines.append(
        "|---------|-------------------|-----------------|-------------|"
    ...
    for ds_name in df["dataset"].unique():
        ...
        report_lines.append(
            f"| {ds_name} | {_fmt(n2n)} | {_fmt(a2n)} | {_fmt(m2n)} | " f"{_fmt(m2c)} |"
        )
    ...
    report_lines.append(
        f"Uncertainty reported as mean ± std over {df['n_seeds'].iloc[0]} "
        f"random seeds."
    )
    report_lines.append(
        "Larger std indicates training instability or sensitivity to "
        "initialization.\n"
    )
...
def _run_severity_sweeps(
    ...
    for regime_name, train_fn in regimes.items():
        ...
        for mult in severity_multipliers:
            ...
            logger.info(
                f"    {regime_name} @ {mult:.1f}x: " f"Acc={acc:.4f}, F1={f1:.4f}"
            )
