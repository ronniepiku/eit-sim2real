...
def _predict_cnn(
    model: EITConv1D, X: np.ndarray, device: str | None = None
) -> np.ndarray:
    if device is None:
        device = _get_device()
...
def run_gaussian_only_evaluation(
    ...
    clean_acc_clean = float(
        accuracy_score(ds.y_test, _predict_cnn(cnn_clean, ds.X_test, device))
    )
    clean_acc_gauss = float(
        accuracy_score(ds.y_test, _predict_cnn(cnn_gauss, ds.X_test, device))
    )
    clean_acc_full = float(
        accuracy_score(ds.y_test, _predict_cnn(cnn_full, ds.X_test, device))
    )
    ...
    for i, (model_name, data) in enumerate([(k, results[k]) for k in models.keys()]):
        ax.plot(
            snr_levels,
            data["accuracies"],
            f"{markers[i]}-",
